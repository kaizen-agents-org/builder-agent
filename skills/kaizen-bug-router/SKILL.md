...
- Apply execution or selection labels only for trusted organization automation or after explicit maintainer approval; an external reporter's request alone is not authorization. For an authorized request to queue, approve, run, execute, or put the issue on the Kaizen Loop, require and add both the repository's execution authorization label and its configured `issues.selection.includeLabel`.
...
- If either required label is unavailable or cannot be applied, report the missing or unapplied label and do not claim the issue is executable. For an authorized immediate-execution request, report the next command, such as `kaizen fix <issue>`, only after both labels have been applied successfully.
