...
export async function createBuildInfo(root = packageRoot): Promise<GeneratedBuildInfo> {
...
export async function readCliBuildInfo(root = packageRoot): Promise<CliBuildInfo> {
  let generated: GeneratedBuildInfo;

  try {
    generated = JSON.parse(await readFile(join(root, "dist", "build-info.json"), "utf8")) as GeneratedBuildInfo;
  } catch {
...
async function calculateSourceHash(root: string): Promise<string> {
...
async function unknownBuildInfo(root: string): Promise<CliBuildInfo> {
  let version = "unknown";
  try {
