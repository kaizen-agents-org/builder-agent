...
export function createBuildResult(input) {
    const { status, iterations, planSummary, changedFiles, review, residualNotes, discoveredIssues, threshold } = input;
...
function normalizeDiscoveredIssue(item, index) {
    ...
    assertAllowedKeys(item, new Set(["title", "body", "expected", "evidence", "repo", "severity", "labels"]), `Build result discoveredIssues[${index}]`);
